...
def build_followup(data, question):
    ...
    facts = (f"· 분야: {data['분야']} / 담당 캐릭터: {data['캐릭터']} ({data['캐릭터소개']})\n"
             ...
             f"· 신살: {sinsal}\n"
             f"· 기문 최적 시기(좋은 순서대로): {', '.join(str(m)+'월' for m in qm['최적월'][:3])} "
             f"(리포트에서 이 달들을 좋은 시기로 짚었으니, 이 달들에 대해선 긍정적으로 일관되게 답할 것). 방위/성격 등 요소: "
...
def build_naming_prompt(result):
    ...
    return (
        ...
        "3. 마지막에 '어떤 이름이든 부모 마음이 제일 중요하다'는 따뜻한 응원.\n"
...
def build_analysis_prompt(r):
    ...
    return (
        ...
        "4. ★개명(이름 바꾸기)이나 이름을 고치라는 말은 절대 하지 말 것. 아쉬운 점이 있어도 겁주지 말고, '부족한 부분은 살면서 노력과 마음으로 충분히 채울 수 있다', '결국 이름은 불러주는 사람들의 애정이 제일 중요하다'는 식으로 따뜻하게 마무리. 좋은 이름이면 진심으로 축하.\n"
